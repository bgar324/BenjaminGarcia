    ...
    ">
      ...
      <div className="flex flex-col justify-between md:w-2/3">
        <div className="flex flex-col">
          ...
          {note && <span className="text-[#d87474] dark:text-[#f29ca3] text-sm">{note}</span>}

          <p className={`text-gray-600 dark:text-slate-300 text-sm md:text-base ${isExpanded ? "" : "line-clamp-2"}`}>
            {description}
          </p>
          <button
        ...
        {tags && (
          <div className="flex flex-wrap gap-2 mt-4">
            {tags.map((tag, index) => (
              <span
                key={index}
                className="px-3 py-1 bg-gray-100 dark:bg-slate-800 text-gray-700 dark:text-slate-200 rounded-full text-xs"
              >
                {tag}
              </span>
            ))}
